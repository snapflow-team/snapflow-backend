...
> Стек: NestJS + CQRS + Prisma + Stripe (Checkout Sessions, Subscriptions, Invoices). Авторизация — через `RemoteAuthGuard`, который ходит в `snapflow-core` за пользователем по access-токену.
...
Сервис на текущий момент поддерживает два плана (см. `apps/payments/src/setup/configuration/business-rules-settings.ts`):
...
Сейчас единственный провайдер — `STRIPE` (enum `PaymentProvider`). Архитектурно сервис подготовлен под несколько провайдеров, но публично наружу торчит только Stripe Checkout Session.

---

## 2. Жизненный цикл подписки

Состояния подписки в БД (enum `SubscriptionStatus`):

- `PENDING` — пользователь начал чекаут, но ещё не оплатил (ожидаем `checkout.session.completed`).
...
Маппинг событий Stripe → бизнес-логика (для справки QA, см. `stripe-events.constants.ts`):

| Stripe event | Что происходит в системе |
| --- | --- |
| `checkout.session.completed` | подписка активируется (`PENDING` → `ACTIVE`) |
| `checkout.session.expired` | подписка отменяется (`PENDING` → `CANCELLED`) |
| `invoice.payment_succeeded` | подписка успешно продлена Stripe |
| `invoice.payment_failed` | подписка перешла в `PAST_DUE` |
| `customer.subscription.deleted` | подписка завершена и удалена (`ACTIVE`/`PAST_DUE` → `CANCELLED`) |
...
**Использование на фронте:** показать пользователю карточки тарифов и подписать в кнопку «Оформить» соответствующий `id`.
...
| --- | --- |
| `4000 0025 0000 3155` | Требует 3DS-аутентификации каждый раз. Используем для проверки сценария «успешная оплата после челленджа». |
| `4000 0027 6000 3184` | Требует 3DS, но **аутентификация падает** — платёж отклонён. |
| `4000 0082 6000 3178` | 3DS требуется, у пользователя нет средств после прохождения челленджа (`insufficient_funds`). |
| `4000 0038 0000 0446` | Аутентификация требуется (frictionless flow). |

### 5.3 Отказы платежа (`card_declined` и др.)
...
### 5.4 Поведение в Checkout Session (важно для QA)
...
- Swagger: `http://<host>:<port>/api/v1/<SWAGGER_PATH>` (в dev — без basic-auth).
- Stripe test cards (официальная страница, постоянно обновляется): https://docs.stripe.com/testing
- Stripe CLI: https://docs.stripe.com/stripe-cli
- Stripe events reference: https://docs.stripe.com/api/events/types
