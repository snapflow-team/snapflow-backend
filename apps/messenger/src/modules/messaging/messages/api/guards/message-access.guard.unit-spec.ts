...
describe('MessageAccessGuard (unit)', () => {
  ...
  beforeEach(async () => {
    ...
    guard = module.get(MessageAccessGuard);
  });
  ...
  it('должен вернуть 401, если пользователь не аутентифицирован', async () => {
    await expect(guard.canActivate(createContext({ messageId: '100' }))).rejects.toBeInstanceOf(
      UnauthorizedException,
    );
  ...
  it('должен вернуть 400 при невалидном messageId', async () => {
    await expect(guard.canActivate(createContext({ messageId: 'abc' }, 1))).rejects.toBeInstanceOf(
      BadRequestException,
    );
  ...
  it('должен вернуть 400, если в маршруте нет messageId', async () => {
